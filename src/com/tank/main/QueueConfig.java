...
public class QueueConfig {
	...
	public void configQosPathPorts(List<Integer> path){
		...
		for(int i = 0 ;i < path.size() - 1; ++i){
			...
			temps = "{\"ovsdb:name\":\""+ topo.ovsTpDbNameMaps.get(link.getSrcTp()) + "\",\"tp-id\":\""+topo.portMaps.get(link.getSrcTp())+
					"\",\"qos\":\""+ qos.QosUUID.get(dbEn.getIndexByIP(topo.ovsdbIpMaps.get(link.getSrcNodeId())))+"\"}";
			RequestManager.request(s[0].replace("{ovsdb-ip}", topo.ovsdbIpMaps.get(link.getSrcNodeId())).replace("{ovsdb-port}", ovsdbPort[0])+
					topo.nodeMaps.get(link.getSrcNodeId())+endStr+topo.portMaps.get(link.getSrcTp()) , s[1], sb.toString()+temps+"]}");

			temps = "{\"ovsdb:name\":\""+ topo.ovsTpDbNameMaps.get(link.getDstTp()) + "\",\"tp-id\":\""+topo.portMaps.get(link.getDstTp()) +
					"\",\"qos\":\""+qos.QosUUID.get(dbEn.getIndexByIP(topo.ovsdbIpMaps.get(link.getDstNodeId())))+"\"}";
			RequestManager.request(s[0].replace("{ovsdb-ip}", topo.ovsdbIpMaps.get(link.getDstNodeId())).replace("{ovsdb-port}", ovsdbPort[0])+
					topo.nodeMaps.get(link.getDstNodeId())+endStr+topo.portMaps.get(link.getDstTp()) , s[1], sb.toString()+temps+"]}");
